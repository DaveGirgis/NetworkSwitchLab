...
| `10.0.23.0/30` | R2–R3 WAN link | Point-to-point between R2 and R3 |

### Addressing Convention

- `192.168.X.0/24` — LAN subnet where X is the router number
- `10.0.XY.0/30` — WAN subnet where XY identifies the router pair (12 = R1↔R2, 23 = R2↔R3)
...
| R2 | Fa0/1 | 10.0.23.2 | 255.255.255.252 | R2 side of R2–R3 WAN |
| R2 | Loopback0 | 192.168.2.1 | 255.255.255.0 | R2 simulated LAN |
| R3 | Fa0/1 | 10.0.23.3 | 255.255.255.252 | R3 side of R2–R3 WAN |
...
OSPF uses wildcard masks (the inverse of the subnet mask). A `/24` uses wildcard `0.0.0.255`; a `/30` uses wildcard `0.0.0.3`.
...
| R2 | 10.0.23.0 | 0.0.0.3 | 0 |
| R3 | 192.168.3.0 | 0.0.0.255 | 0 |
| R3 | 10.0.23.0 | 0.0.0.3 | 0 |
...
| R2 | 10.0.23.0 | 0.0.0.3 |
| R3 | 192.168.3.0 | 0.0.0.255 |
| R3 | 10.0.23.0 | 0.0.0.3 |
